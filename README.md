...
EG (ECG), EX (Study Drug Administration), CM (Concomitant Medications)** plus each **lab panel
as its own dataset** — pulled **live from DataBricks** (study **FIVE**) and deployed to
**Posit Connect Cloud**.
...
`tm_g_bivariate` (Medication Counts) — the non-CDISC-safe subset of `teal.modules.general`.
